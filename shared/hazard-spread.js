...
export const SPREAD = Object.freeze({
  //        r0   rMax  tau(ms)  설명
  // **r0 는 «작은 불» 이 아니라 «눈에 띄는 최소» 다.**
  //
  // 처음엔 2.2 로 뒀다. 갓 난 불은 작아야 맞으니까. 그런데 화면에서 반지름
  // 7px 이라 관제를 화면 전체로 띄워도 점 하나였고, 25초가 지나도 «눌렀는데
  // 아무 일도 안 일어난다» 로 보였다. 실제로는 들어가 있었다.
  //
  // 관제 화면에서 «불이 났다» 는 **누른 즉시** 보여야 한다. 그게 이 화면이
  // 존재하는 이유다. 번지는 것은 그 다음 이야기고, r0 을 올려도 rMax 까지
  // 자라는 것은 그대로다 — 처음 크기만 «보이는 값» 으로 올린다.
  fire:  { r0: 6, rMax: 15, tauMs: 70_000 },
  smoke: { r0: 8, rMax: 24, tauMs: 45_000 },   // 더 크고 더 빠르다
